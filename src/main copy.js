...
function showNavigation() {
  ...
  navEl.innerHTML = `
    <li class="shop-button selected" onclick="filterByCategory(this, 'Alla')">Alla</li>
  `; // Lägg till "Alla" som standard vald
  categories.forEach((category) => {
    navEl.innerHTML += `
      <li class="shop-button" onclick="filterByCategory(this, '${category}')">${category}</li>
    `;
  });
}
...
function filterByCategory(element, category) {
  // Ta bort "selected"-klassen från alla kategorier
  ...
  // Lägg till "selected" till den valda kategorin
